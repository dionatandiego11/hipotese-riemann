"""
Reavaliação da força da cota P-RH com Lema 5′ (polos no trecho compacto por oscilação), conforme results/etapa11_r5/DECLARACAO_PRH2.md (hash verificado). Sem dados de zeros.
...
raw = open("results/etapa11_r5/DECLARACAO_PRH2.md", "rb").read()
assert hashlib.sha256(raw).hexdigest() == open("results/etapa11_r5/DECLARACAO_PRH2.sha256").read().split()[0]
iv.dps = 30
TAU = 1e-6

# ---- c1, c2: enclosures já obtidos (prh_forca_resumo.json) ----
...
def bound(a, L, nus, d1, Dl):
    ...
    for nu in nus:
        ...
        Pol = (iv.exp(-U1/2)*L0
               ...
               + 2*kap*a*(4*iv.exp(U2/2)/(nu*(lam*s2)**3) + 2*iv.sqrt(2*PI)*(lam*nu)**(-3)*EPS0**5*iv.exp(-(nu**2 - iv.mpf(1)/4)/(2*EPS0**2))))
        zeros += Z/2; poles += Pol
...
with open("results/etapa11_r5/prh2_forca_tabela.csv", "w", newline="") as fh:
...
json.dump(summ, open("results/etapa11_r5/prh2_forca_resumo.json", "w"), indent=1)
