...
conforme results/etapa11_r5/DECLARACAO_CERT.md e os adendos DECLARACAO_CERT_CORRECAO1.md e DECLARACAO_CERT_CORRECAO2.md
e DECLARACAO_CERT_RECONF_C1C2.md (hashes verificados). Execução 4: c₁ e c₂ recalculados internamente (sem strings herdadas).
Desde a execução 3: extremos exatos (Fraction); só +, −, ×, ÷ em binary64 com nextafter; finitude verificada;
exportações conferidas por frações exatas.
...
ADD = "results/etapa11_r5/DECLARACAO_CERT_CORRECAO1.md"
assert hashlib.sha256(open(ADD, "rb").read()).hexdigest() == open("results/etapa11_r5/DECLARACAO_CERT_CORRECAO1.sha256").read().split()[0]
ADD2 = "results/etapa11_r5/DECLARACAO_CERT_CORRECAO2.md"
assert hashlib.sha256(open(ADD2, "rb").read()).hexdigest() == open("results/etapa11_r5/DECLARACAO_CERT_CORRECAO2.sha256").read().split()[0]
ADD3 = "results/etapa11_r5/DECLARACAO_CERT_RECONF_C1C2.md"
assert hashlib.sha256(open(ADD3, "rb").read()).hexdigest() == open("results/etapa11_r5/DECLARACAO_CERT_RECONF_C1C2.sha256").read().split()[0]
iv.dps = 30
...
class I:
    ...
    def __init__(self, lo, hi=None):
        ...
        assert np.all(np.isfinite(self.lo)) and np.all(np.isfinite(self.hi)), "valor não finito"   # adendo 2, L5
    def __add__(s, o):
...
# ---------------------------------------------------------------- extremos exatos (adendo 1)
def _q(raw):
    assert raw not in (libmp.finf, libmp.fninf, libmp.fnan), "extremo não finito"
    return Fraction(*libmp.to_rational(raw))
def lo_q(x): return _q(x._mpi_[0])
def hi_q(x): return _q(x._mpi_[1])
def pt_lo(x): return iv.make_mpf((x._mpi_[0], x._mpi_[0]))
def pt_hi(x): return iv.make_mpf((x._mpi_[1], x._mpi_[1]))
def iv_frac(q): return iv.mpf(q.numerator)/iv.mpf(q.denominator)
def float_dn(q):
    f = float(q)
    return f if Fraction(f) <= q else float(np.nextafter(f, -INF))
def float_up(q):
    f = float(q)
    return f if Fraction(f) >= q else float(np.nextafter(f, INF))

PI_INF = None  # definido após PI

def sup_chk(x, digits):
    """sup_str com verificação exata: Fraction(string) ≥ extremo superior (adendo 2, L2)."""
    st = sup_str(x, digits); assert Fraction(st) >= hi_q(x), "exportação abaixo do extremo superior"
    return st

def fin(x):
    assert np.all(np.isfinite(x)), "valor não finito"; return x

def ivf(z):
    """mpmath iv → I escalar, para fora, a partir dos extremos exatos."""
    return I(float_dn(lo_q(z)), float_up(hi_q(z)))
...
# ---------------------------------------------------------------- W exato (mpmath.iv)
PI = iv.pi
PI_INF = float_dn(lo_q(PI))                          # adendo 2, L4
THR = Fraction(1, 1000)
def sinc_near0(x):
    """sinc(x) = sin(πx)/(πx) ∈ 1 − [0, (π x_m)²/6] para |x| ≤ x_m (sin y ≥ y − y³/6, y ≥ 0)."""
    xm = max(abs(lo_q(x)), abs(hi_q(x))); assert xm < THR
    S = (PI*iv_frac(xm))**2/6
    return 1 - iv.make_mpf((libmp.fzero, S._mpi_[1]))

def W_iv(om, L):
    """W(ω) = (L/2)·sinc(v)/(1 − v²), v = ωL/(2π), com extensão contínua (ramos decididos por frações exatas)."""
    v = om*L/(2*PI); L2 = L/2
    vlo, vhi = lo_q(v), hi_q(v); vm = max(abs(vlo), abs(vhi))
    if vm < THR:
        return L2*sinc_near0(v)/(1 - v*v)
    if vlo > 0 or vhi < 0:
        av = v if vlo > 0 else -v
        x = av - 1
        if max(abs(lo_q(x)), abs(hi_q(x))) < THR:
            return L2*sinc_near0(x)/(av*(av + 1))
    den = PI*v*(1 - v*v)
    assert lo_q(den) > 0 or hi_q(den) < 0, "denominador contém 0"
    return L2*iv.sin(PI*v)/den
...
def cell_sums():
    ...
    lo_int = [math.floor(lo_q(x)) for x in Xb]; hi_int = [math.floor(hi_q(x)) for x in Xb]
    N = hi_int[-1]
...
def block(args):
    ...
    RG = matmul(I(R), G)
    E = I(np.eye(2*K)) - RG
    rho = float(fin(np.max(up(isum(I(E.mag()), axis=1).hi))))
    assert rho < 1, rho
    e = float(fin(up(up(rho)/dn(1 - rho))))
    # ---- Passo 3: linhas
    Y = matmul(I(R), Mt)                                   # 94 × 846
    ynorm = isum(I(Y.mag()), axis=1).hi                    # sup ‖y_i‖₁
    ymax_col = np.max(Y.mag(), axis=0)                      # max_i |y_ij|
    a_l1 = fin(up(ynorm[:K] + up(e*np.max(ynorm))))
    rad = up(e*ymax_col)
    ...
    vmin_global = fin(dn(lam.lo*dn(U1 - tmax))); assert vmin_global > 0 and Fraction(float(vmin_global))**2 >= 2   # adendo 2, L3
    ...
    for ci in range(NCELL):
        ...
        q2 = fin(dn(vmn*vmn)); v4 = fin(dn(q2*q2)); assert v4 > 0   # adendo 2, L1: v⁴ por multiplicações dirigidas
        dR = fin(up(12.0/v4))                                  # |R′| ≤ 12|v|⁻⁴
        acc = np.zeros(K)
        for sgn in (-1.0, 1.0):
        ...
        supQ[:, ci] = fin(up(acc + up(deriv*(CELL/2))))
    Sabs = np.zeros(K)
    for ci in range(NCELL):
        Sabs = up(Sabs + up(supQ[:, ci]*cellS_hi[ci]))
    Sabs = fin(up(Sabs/PI_INF))                                  # 1/π com π_inf dirigido (adendo 2, L4)
...
# ---------------------------------------------------------------- c₁ = ‖ϕ′‖_∞, c₂ = ‖ϕ″‖_∞ (adendo 3, Método B)
def phi_derivs(x):
    """Enclausuramentos de ϕ′ e ϕ″ num intervalo x ⊂ (0, 1), ϕ = f(x)/(f(x)+f(1−x)), f = e^{−1/x}."""
    y = 1 - x
    f = lambda z: iv.exp(-1/z)
    f1 = lambda z: iv.exp(-1/z)/z**2
    f2 = lambda z: iv.exp(-1/z)*(1 - 2*z)/z**4
    S = f(x) + f(y); N = f1(x)*f(y) + f(x)*f1(y); Np = f2(x)*f(y) - f(x)*f2(y); Sp = f1(x) - f1(y)
    return N/S**2, (Np*S - 2*N*Sp)/S**3

def abs_hi(z):
    return max(abs(lo_q(z)), abs(hi_q(z)))

def phi_constants(NSUB=20000):
    A0 = Fraction(1, 100); A1 = Fraction(99, 100); step = (A1 - A0)/NSUB
    best1 = (Fraction(0), None); best2 = (Fraction(0), None)
    prev_hi = None; first_lo = None; gaps = 0
    for i in range(NSUB):
        xl = iv_frac(A0 + step*i); xr = iv_frac(A0 + step*(i + 1))
        x = iv.make_mpf((xl._mpi_[0], xr._mpi_[1]))                 # envoltória exata [inf x_i, sup x_{i+1}]
        if first_lo is None: first_lo = lo_q(x)
        if prev_hi is not None and prev_hi < lo_q(x): gaps += 1
        prev_hi = hi_q(x)
        d1, d2 = phi_derivs(x)
        a1, a2 = abs_hi(d1), abs_hi(d2)
        if a1 > best1[0]: best1 = (a1, (d1, abs(d1)))
        if a2 > best2[0]: best2 = (a2, (d2, abs(d2)))
    cover = {"lacunas": gaps, "primeiro_inf_le_1/100": first_lo <= A0, "ultimo_sup_ge_99/100": prev_hi >= A1}
    assert gaps == 0 and cover["primeiro_inf_le_1/100"] and cover["ultimo_sup_ge_99/100"], cover
    # pontas (0, δ] e, por simetria ϕ(1−x) = 1 − ϕ(x), [1−δ, 1): cotas grosseiras (§II.20)
    delta = iv_frac(Fraction(1, 100))
    fmax = iv.exp(-1/delta); f1max = iv.exp(-1/delta)/delta**2; f2max = iv.exp(-1/delta)/delta**4
    gmin = iv.exp(-1/(1 - delta)); hmax = iv.exp(-1)/(1 - delta)**2; h2max = 3*iv.exp(-1)/(1 - delta)**4
    Nb = f1max + fmax*hmax; Npb = f2max + fmax*h2max; Spb = f1max + hmax
    tip1 = Nb/gmin**2; tip2 = Npb/gmin**2 + 2*Nb*Spb/gmin**3
    t1, t2 = abs_hi(tip1), abs_hi(tip2)
    c1q = max(best1[0], t1); c2q = max(best2[0], t2)
    info = {**cover, "malha_c1_sup": str(best1[0]), "malha_c2_sup": str(best2[0]), "ponta_c1_sup": str(t1), "ponta_c2_sup": str(t2)}
    return c1q, c2q, info

C1Q, C2Q, C12_INFO = phi_constants()
c1 = iv_frac(C1Q); c2 = iv_frac(C2Q)
c1 = iv.make_mpf((c1._mpi_[1], c1._mpi_[1])) if hi_q(c1) >= C1Q else None
c2 = iv.make_mpf((c2._mpi_[1], c2._mpi_[1])) if hi_q(c2) >= C2Q else None
assert c1 is not None and c2 is not None and lo_q(c1) >= C1Q and lo_q(c2) >= C2Q
...
def BRH2(a, L, nus, d1=D1, Dl=DELTA):
    ...
    def mn(x, y): return pt_hi(x) if hi_q(x) <= hi_q(y) else pt_hi(y)   # majorante do mínimo
...
if __name__ == "__main__":
    ...
    t0 = time.time()
    S, sinfo = cell_sums()
    cellS_hi = np.array([float_up(hi_q(x)) for x in S])
    ...
    for b in sorted(res, key=lambda r: (r["versao"], r["bloco"])):
        ...
        for k, c in enumerate(catalog):
            ...
            rows.append({"versao": b["versao"], "bloco": b["bloco"], "p": c["prime"], "r": c["repetition"], "eligible": elig[kk],
                         ...
                         "B_rel_sup": sup_chk(Bv/ck_inf, 15), "Sabs_rel_sup": sup_chk(iv.mpf(b["Sabs_sup"][k])/ck_inf, 15),
                         "Sabs_rel_float": float(theta[kk]["S_abs"])/float(theta[kk]["abs_c"]),
                         "total_sup": tot_s, "orcamento2_hibrido": float(prh3[kk]["orcamento2"]),
                         "certificado": hi_q(tot) <= Fraction(1, 10**6) and b["rho"] < 1 and b["vmin"] > 0 and Fraction(b["vmin"])**2 >= 2 and sinfo["crivo_conferido"]})
    ...
    summ = {"declaracao_sha256": open("results/etapa11_r5/DECLARACAO_CERT.sha256").read().split()[0],
            "adendo1_sha256": open("results/etapa11_r5/DECLARACAO_CERT_CORRECAO1.sha256").read().split()[0],
            "adendo2_sha256": open("results/etapa11_r5/DECLARACAO_CERT_CORRECAO2.sha256").read().split()[0],
            "adendo3_sha256": open("results/etapa11_r5/DECLARACAO_CERT_RECONF_C1C2.sha256").read().split()[0], "execucao": 4,
            "c1_sup": {"fracao_exata_sup": str(hi_q(c1)), "decimal15": sup_chk(c1, 15), "decimal6": sup_chk(c1, 6)},
            "c2_sup": {"fracao_exata_sup": str(hi_q(c2)), "decimal15": sup_chk(c2, 15), "decimal6": sup_chk(c2, 6)},
            "c12_info": C12_INFO,
            "diagnostico_strings_antigas": {"c1_novo_le_2.000982": hi_q(c1) <= Fraction("2.000982"), "c2_novo_le_9.854620": hi_q(c2) <= Fraction("9.854620")},
