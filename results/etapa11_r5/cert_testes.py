"""Testes obrigatórios T1–T6 (adendo 1), T7–T8 (adendo 2) e T9 (adendo 3); devem passar antes da reexecução."""
...
# T7 (adendo 2): sup_str/inf_str contra extremos exatos
sys.path.insert(0, "results/etapa11_3b")
from interval_export import sup_str, inf_str
f7 = []; c7 = 0
cases = []
for x in rng.uniform(-1, 1, 2500):
    e10 = int(rng.integers(-300, 300)); cases.append(iv.mpf(repr(float(x)))*iv.mpf(10)**e10/3)
for x in rng.uniform(0, 1, 2500):
    cases.append(iv.exp(iv.mpf(repr(float(x*50 - 25)))) - iv.mpf("1e-7"))
for d in [6, 12, 15]:
    base_ = iv.mpf(1)/iv.mpf(10)**d
    for j in range(1, 200):
        cases += [base_*j, base_*j + iv.mpf("1e-28"), base_*j - iv.mpf("1e-28"), -(base_*j)]
cases += [iv.mpf("1e-6"), iv.mpf(1)/iv.mpf(10**6)] + zs[:47]
for z in cases:
    for d in [6, 12, 15]:
        c7 += 1
        if Fraction(sup_str(z, d)) < cc.hi_q(z): f7.append(("sup", d, str(z)))
        if Fraction(inf_str(z, d)) > cc.lo_q(z): f7.append(("inf", d, str(z)))
res["T7"] = {"casos": c7, "falhas": f7[:10], "n_falhas": len(f7)}

# T8 (adendo 2): v⁴ dirigido
f8 = 0
for x in np.concatenate([rng.uniform(1.4, 20, 5000), rng.uniform(1e3, 1e4, 5000)]):
    q2 = cc.dn(x*x); v4 = cc.dn(q2*q2)
    if Fraction(float(v4)) > Fraction(float(x))**4: f8 += 1
res["T8"] = {"casos": 10000, "n_falhas": f8}

# T9 (adendo 3): c₁, c₂ recalculados
import inspect
old = iv.dps; iv.dps = 50
f9 = []
pts = [Fraction(int(k), 10**4) for k in rng.integers(1, 10**4, 9000)]
pts += [Fraction(1, 10**6)*j for j in range(1, 200)] + [1 - Fraction(1, 10**6)*j for j in range(1, 200)]
pts += [Fraction(1, 2) + Fraction(int(k) - 300, 10**5) for k in range(0, 600)]
smax1 = Fraction(0); smax2 = Fraction(0)
for q in pts:
    if not (0 < q < 1): continue
    d1, d2 = cc.phi_derivs(cc.iv_frac(q))
    l1 = max(Fraction(0), min(abs(cc.lo_q(d1)), abs(cc.hi_q(d1))) if cc.lo_q(d1)*cc.hi_q(d1) > 0 else Fraction(0))
    l2 = max(Fraction(0), min(abs(cc.lo_q(d2)), abs(cc.hi_q(d2))) if cc.lo_q(d2)*cc.hi_q(d2) > 0 else Fraction(0))
    smax1 = max(smax1, l1); smax2 = max(smax2, l2)
    if l1 > cc.hi_q(cc.c1): f9.append(("c1", str(q)))
    if l2 > cc.hi_q(cc.c2): f9.append(("c2", str(q)))
iv.dps = old
srcphi = inspect.getsource(cc.phi_constants) + inspect.getsource(cc.phi_derivs)
tip_ok = not re.search(r"float\(|(?<![\w.])mpf\(|np\.|math\.", srcphi)
src_all = open("results/etapa11_r5/cert_calculo.py").read()
no_inherit = ("prh_forca_resumo" not in src_all) and ('["c1_sup"]' not in src_all) and ('["c2_sup"]' not in src_all) and ("_prev" not in src_all)
res["T9"] = {"a_cobertura": cc.C12_INFO, "b_pontos": len(pts), "b_falhas": f9[:10], "b_n_falhas": len(f9),
             "b_max_amostral_c1": float(smax1), "b_max_amostral_c2": float(smax2),
             "b_c1_sup": float(cc.hi_q(cc.c1)), "b_c2_sup": float(cc.hi_q(cc.c2)),
             "c_so_intervalar": tip_ok, "d_sem_heranca": no_inherit}
t9ok = (cc.C12_INFO["lacunas"] == 0 and len(f9) == 0 and tip_ok and no_inherit)

ok = (t9ok and res["T7"]["n_falhas"] == 0 and res["T8"]["n_falhas"] == 0 and res["T1"]["n_falhas"] == 0 and not res["T2"]["falhas"] and res["T3"]["n_falhas"] == 0 and not res["T4"]["falhas"]
