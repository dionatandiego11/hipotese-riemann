...
def measure_window_response(L: float, window: str = "hann") -> Dict[str, Any]:
    ...
    # m4-v3: raízes em unidades adimensionais v = ω L/2π com tolerância de precisão de máquina
    # (m4-v2 usava xtol absoluto padrão em ω, erro relativo de até 2,5e-10 na FWHM; ver AUDITORIA_INDEPENDENTE §4)
    ...
    v_half = brentq(lambda x: abs(float(window_response(x * unit, L, window))) - 0.5 * W0, 1e-9, v_null,
                    xtol=1e-15, rtol=4 * np.finfo(float).eps, maxiter=200)
    half = v_half * unit
...
def local_gauss_legendre_panels(L: float, panel_length: float, order: int = 8) -> Tuple[np.ndarray, np.ndarray]:
    """
    m4-v3: Gauss-Legendre composto em coordenadas LOCAIS x in [0, L] (deslocamento a partir de A), com n painéis
    de largura exatamente L/n e pesos idênticos por painel. Evita o arredondamento com padrão regular que surgia ao
    formar nós e pesos a partir de números grandes (A ~ 1e4–1e5): diferenças de linspace(A, B) e E − E_c.
    """
    n_panels = max(1, int(math.ceil(L / panel_length)))
    h = L / n_panels
    xg, wg = np.polynomial.legendre.leggauss(order)
    offsets = ((np.arange(n_panels) + 0.5)[:, None] * h + 0.5 * h * xg[None, :]).ravel()
    weights = np.tile(0.5 * h * wg, n_panels)
...
class OscillatoryTransform:
    ...
    def __init__(
        ...
        self._quad_nodes = self.A + self._quad_offsets          # só para avaliar a densidade (função suave)
        self._quad_u = self._quad_offsets - 0.5 * self.L        # E − E_c em coordenadas locais (fases)
        self._quad_coeffs = (
            self._window_at_offsets(self._quad_offsets)
            * smooth_density(self._quad_nodes, density)
            * self._quad_weights
        )
        # m4-v3: termo suave por soma direta sobre os nós de quadratura. Em m4-v2 usava NUFFT, cujo erro relativo
        # (~1e-12 da soma dos |coeficientes| ~ L·dbar/2) deixava um piso absoluto de alguns 1e-9 num termo que
        # cancela até ~1e-8 (ensaio prévio de m4-v3 em dados já analisados; ver PROTOCOLO §11.2).
        self.smooth = nufft_uniform_t(self._quad_u, self._quad_coeffs, self.t_grid[0], self.dt, self.n_t)

    def _window_at_offsets(self, offsets: np.ndarray) -> np.ndarray:
        """Janela avaliada pela fração local x/L (sem subtrair números grandes)."""
        coeffs = window_coefficients(self.window)
        frac = np.asarray(offsets, dtype=np.float64) / self.L
        w = np.zeros_like(frac)
        for k, a in enumerate(coeffs):
            w += ((-1) ** k) * a * np.cos(TWO_PI * k * frac)
        return w
    ...
    def evaluate(self, levels: np.ndarray, t_values: np.ndarray) -> np.ndarray:
        ...
        smooth = direct_exponential_sum(self._quad_u, self._quad_coeffs, t_values, chunk=16)
        return disc - smooth
    ...
    def diagnostics(self, levels: np.ndarray, n_check: int = 64, seed: int = 0) -> Dict[str, Any]:
        ...
        off2, weights2 = local_gauss_legendre_panels(self.L, 0.5 * self.panel_length, self.quad_order)
        nodes2 = self.A + off2
        coeffs2 = self._window_at_offsets(off2) * smooth_density(nodes2, self.density) * weights2
        smooth2 = nufft_uniform_t(off2 - 0.5 * self.L, coeffs2, self.t_grid[0], self.dt, self.n_t)
        quad_err = float(np.max(np.abs(smooth2 - self.smooth)))

        alt = "theta" if self.density == "rvm" else "rvm"
        coeffs_alt = self._window_at_offsets(self._quad_offsets) * smooth_density(self._quad_nodes, alt) * self._quad_weights
        smooth_alt = nufft_uniform_t(self._quad_u, coeffs_alt, self.t_grid[0], self.dt, self.n_t)
