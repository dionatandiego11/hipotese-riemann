...
def validate_with_mpmath(
    ...
    for n in indices:
        ...
        comparisons.append({
            ...
            "within_declared_error": bool(abs_err <= declared_source_error),
            "zeta_residual_magnitude": zeta_abs,
        })

    # Critério: erro absoluto <= erro declarado pela fonte (sem fator adicional; revisão de 13/09/2026)
    all_passed = bool(max_abs_err <= declared_source_error)
    ...
    report = {
        ...
        "tolerance_rule": "abs_error <= declared_source_error",
...
def dataset_paths(config: Dict[str, Any]) -> Dict[str, Path]:
    """
    Caminhos de manifesto e validação identificados pelo conjunto de dados (`dataset_id`),
    para que perfis diferentes não sobrescrevam registros um do outro.
    """
    if "dataset_id" not in config:
        raise KeyError("A configuração precisa de `dataset_id` (identidade do conjunto de dados processado).")
    ds = str(config["dataset_id"])
    raw_dir = Path(config.get("raw_data_dir", "data/raw"))
    proc_dir = Path(config.get("processed_data_dir", "data/processed"))
    return {
        "raw_file": raw_dir / config.get("raw_filename", "zeros1"),
        "processed_file": proc_dir / config["processed_csv"],
        "manifest": raw_dir / "manifests" / f"{ds}.json",
        "validation": proc_dir / "validation" / f"{ds}.json",
    }


def check_validation_matches_dataset(validation: Dict[str, Any], processed_file: Path, n_loaded: int) -> None:
    """Recusa usar uma validação produzida para outro arquivo ou outra contagem de zeros."""
    sha = compute_file_sha256(processed_file)
    if validation.get("processed_sha256") != sha:
        raise RuntimeError(f"Validação não corresponde ao arquivo carregado {processed_file} "
                           f"(sha {validation.get('processed_sha256')} != {sha}).")
    if validation.get("n_zeros_in_file") != n_loaded:
        raise RuntimeError(f"Validação feita com {validation.get('n_zeros_in_file')} zeros; carregados {n_loaded}.")
