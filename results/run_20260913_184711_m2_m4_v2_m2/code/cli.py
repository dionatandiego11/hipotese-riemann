...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    config = load_config(args.config)
    lock_record = None
    if config.get("requires_lock", False):
        try:
            lock_record = require_matching_lock(args.config)
...
def current_protocol_fingerprint(config_path: str) -> Dict[str, Any]:
    """`lock_scope = "package"` congela todos os módulos do pacote (inclui M2, critérios e relatórios)."""
    cfg = load_config(config_path)
    scope = cfg.get("lock_scope", "m3_modules")
    if scope == "package":
        fp = code_fingerprint()
    elif scope == "m3_modules":
        fp = code_fingerprint(M3_PROTOCOL_MODULES)
    else:
        raise ValueError(f"lock_scope desconhecido: {scope!r}")
    return {"config_sha256": compute_file_sha256(config_path), "lock_scope": scope, "modules_sha256": fp["modules"], "combined_sha256": fp["combined_sha256"]}


def require_matching_lock(config_path: str) -> Dict[str, Any]:
    lock = lock_path_for(config_path)
    if not lock.exists():
        raise PermissionError(f"Execução exige protocolo congelado ({lock}). Execute 'protocol freeze'.")
    with open(lock, encoding="utf-8") as f:
        record = json.load(f)
    fp = current_protocol_fingerprint(config_path)
    if fp["config_sha256"] != record["config_sha256"] or fp["combined_sha256"] != record["combined_sha256"]:
        raise PermissionError("Configuração ou código diferem do protocolo congelado. Execução recusada.")
    return record
...
def cmd_m3(args: argparse.Namespace) -> int:
    ...
    if locked & set(requested):
        try:
            ...
            logger.error(f"Blocos {sorted(locked & set(requested))}: {exc}")
            return 3
    ...
    if "criteria" in cfg:
        ...
        family = evaluate_m3_family(results, cfg["criteria"])
        family["blocks_evaluated"] = requested
        family["complete_family"] = sorted(requested) == sorted(b["name"] for b in cfg["blocks"])
        with open(run_dir / "family_summary.json", "w", encoding="utf-8") as f:
            json.dump(family, f, indent=2, ensure_ascii=False)
    write_m3_run_report(run_dir, manifest, results, family)
