...
Verificações V1–V4 da declaração em `verificacoes`.
...
def v4_reality(ch: Character, t_max: float, n_points: int = 10, rel_tol: float = 1e-20) -> Dict:
    rows = []
    for t in np.linspace(10.0, max(t_max, 11.0), n_points):
        with mp.workdps(DEFAULT_DPS):
            z = Z_complex(float(t), ch)
            rel = float(abs(mp.im(z)) / abs(z)) if z != 0 else 0.0
        rows.append({"t": float(t), "imag_over_abs": rel, "ok": rel <= rel_tol})
    return {"rel_tol": rel_tol, "points": rows, "passed": all(r["ok"] for r in rows)}
...
def main(argv=None) -> int:
    ...
    p.add_argument("--carater", required=True, choices=sorted(CHARACTERS))
    p.add_argument("--n", type=int, required=True, help="número de zeros (piloto: 500; completo: 12000)")
