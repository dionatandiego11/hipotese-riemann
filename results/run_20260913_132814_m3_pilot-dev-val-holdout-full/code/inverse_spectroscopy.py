...
def run_block(
    ...
    try:
        ...
        for kind, child in (("shuffle", ss_shuffle), ("gue", ss_gue)):
            ...
            logger.info(f"[{name}] nulo {kind}: B_sigma={n_sigma} B_limiar={n_thr} B_escore={n_score} limiar z={thr:.4f} "
    ...
    for p in peaks:
        ...
        p["detected_secondary"] = bool(p["z_secondary"] >= nulls[secondary]["threshold"])
        p["fwhm_instrument"] = fwhm

    blind_csv = bdir / "blind_peaks.csv"
    cols = ["grid_period", "period", "fwhm_instrument", "z", "p_adjusted_primary", "detected_primary",
    ...
    n_refined_would_differ = int(sum((p["z_refined"] >= thr_p) != p["detected_primary"] for p in peaks))
    freeze = {
    ...
    logger.info(f"[{name}] congelado: {freeze['n_detected_primary']} detecções ({primary}); "
...
def arithmetic_comparison(ar, tr, g, peaks, nulls, primary, tol, res_sep, bdir, cfg, ss_prec, ss_phase) -> Dict[str, Any]:
    ...
    # Escore global S calibrado no conjunto "escore", independente do conjunto que fixou o limiar.
    thr_p = nulls[primary]["threshold"]
    null_scores, null_dets = [], []
