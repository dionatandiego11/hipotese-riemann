...
def window_hann(E: np.ndarray, A: float, B: float) -> np.ndarray:
    """Janela de Hann suportada em [A, B] (zero fora do intervalo)."""
    E = np.asarray(E, dtype=np.float64)
    u = (E - A) / (B - A)
    w = 0.5 * (1.0 - np.cos(TWO_PI * u))
    return np.where((u >= 0.0) & (u <= 1.0), w, 0.0)


def hann_response(omega: np.ndarray | float, L: float) -> np.ndarray | float:
    """
    Resposta analítica W(omega) = int_{-L/2}^{L/2} w(u) e^{-i u omega} du da janela de Hann
    centrada; é real e par. W(0) = L/2.
        W = (L/2) sinc(v) + (L/4) [sinc(v - 1) + sinc(v + 1)],  v = omega L / (2 pi),
    com sinc(x) = sin(pi x)/(pi x).
    """
    v = np.asarray(omega, dtype=np.float64) * L / TWO_PI
    return 0.5 * L * np.sinc(v) + 0.25 * L * (np.sinc(v - 1.0) + np.sinc(v + 1.0))


def measure_hann_response(L: float) -> Dict[str, float]:
    """
    Mede numericamente a resposta da janela de Hann de comprimento L em energia:
    FWHM do módulo, primeiro zero, nível do primeiro lóbulo lateral e ganho W(0).
    """
    unit = TWO_PI / L
    W0 = float(hann_response(0.0, L))
    half = brentq(lambda om: abs(hann_response(om, L)) - 0.5 * W0, 1e-9 * unit, 2.0 * unit)
    first_null = 2.0 * unit  # zero exato de W para Hann: v = 2
    om = np.linspace(2.0 * unit, 4.0 * unit, 4001)
    side = float(np.max(np.abs(hann_response(om, L))))
    return {
        "gain_W0": W0,
        "fwhm": float(2.0 * half),
        "fwhm_in_units_2pi_over_L": float(2.0 * half / unit),
        "first_null_offset": first_null,
...
class OscillatoryTransform:
    ...
    def __init__(
        ...
        self.response = measure_hann_response(self.L)
        ...
        self._quad_coeffs = (
            window_hann(self._quad_nodes, self.A, self.B)
    ...
    def _select(self, levels: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        return sel - self.E_c, window_hann(sel, self.A, self.B)
    ...
    def diagnostics(self, levels: np.ndarray, n_check: int = 64, seed: int = 0) -> Dict[str, Any]:
        ...
        coeffs2 = window_hann(nodes2, self.A, self.B) * smooth_density(nodes2, self.density) * weights2
        ...
        coeffs_alt = window_hann(self._quad_nodes, self.A, self.B) * smooth_density(self._quad_nodes, alt) * self._quad_weights
...
def detect_blind_peaks(
    ...
    1. estatística z(t) = |F(t)| / sigma_nulo(t);
    2. máximos locais com z >= candidate_z e separação mínima de uma FWHM medida da janela;
    3. refinamento parabólico de log|F| em torno do máximo;
    4. amplitude complexa no ponto refinado (soma direta se `evaluator` for dado).
    A significância é decidida depois pelo máximo nulo (`calibrate_threshold`, `adjusted_p_values`).
    """
    ...
    sigma = np.interp(t_ref, t_grid, noise_rms)
    ...
    for k in range(len(idx)):
        peaks.append({
            ...
            "null_noise_rms": float(sigma[k]),
            "z": float(np.abs(amp[k]) / sigma[k]),
        })
    return peaks
