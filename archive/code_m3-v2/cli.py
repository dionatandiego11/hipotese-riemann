...
def cmd_data_fetch(args: argparse.Namespace) -> int:
    ...
    proc_file = proc_dir / config.get("processed_csv", "zeros_10k.csv")
    fetch_zeros(url, raw_file)
    zeros, stats = parse_raw_zeros(raw_file, max_zeros=config.get("max_zeros"))
    save_processed_csv(zeros, proc_file)
    manifest = build_data_manifest(raw_file, proc_file, url, stats)
    with open(raw_dir / "data_manifest.json", "w", encoding="utf-8") as f:
        json.dump(manifest, f, indent=2, ensure_ascii=False)
    logger.info(f"Manifesto de dados salvo em {raw_dir / 'data_manifest.json'}.")
    return 0
...
def cmd_data_validate(args: argparse.Namespace) -> int:
    ...
    if not proc_file.exists():
        logger.error(f"{proc_file} não encontrado. Execute 'data fetch' primeiro.")
        return 1
    ...
    with open(proc_dir / "data_validation.json", "w", encoding="utf-8") as f:
        json.dump(report, f, indent=2, ensure_ascii=False)
    if not report.get("all_within_tolerance", False):
        logger.error("Validação mpmath FALHOU: erro superior à tolerância declarada.")
        return 1
    return 0
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    proc_file = Path(config.get("processed_data_dir", "data/processed")) / config.get("processed_csv", "zeros_10k.csv")
    zeros = load_zeros_csv(proc_file)[: config.get("max_zeros")]
...
def cmd_m3(args: argparse.Namespace) -> int:
    ...
    if missing:
        ...
        return 2
    lock_record = None
    if LOCKED_BLOCKS & set(requested):
        lock = lock_path_for(args.config)
        if not lock.exists():
            logger.error(f"Blocos {sorted(LOCKED_BLOCKS & set(requested))} exigem protocolo congelado ({lock}). Execute 'protocol freeze'.")
            return 3
        with open(lock, encoding="utf-8") as f:
