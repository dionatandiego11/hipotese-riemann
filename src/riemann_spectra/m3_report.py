...
def _family_section(family) -> str:
    if not family:
        return ""
    c = family["criteria"]
    lines = [
        ...
        "",
        f"Família completa: {family.get('complete_family')}. Holm entre {family['n_blocks']} blocos, alpha = {c['alpha_family']}. "
        "Blocos contíguos não são independentes.",
        "",
        "| Bloco | C1: p(S) | p(S) Holm | recuperação (claras) | sem corresp. | inconclusivos | C1 bloco | C2: elegíveis | fração ≤ tol. | Q | p(Q) Holm | C2 bloco |",
        "|---|---|---|---|---|---|---|---|---|---|---|---|",
    ]
    for b, v in family["blocks"].items():
        lines.append(
            f"| {b} | {_fmt(v['C1_p_S'])} | {_fmt(v['C1_p_S_holm'])} | {_fmt(v['C1_recovery_clear'])} | {v['C1_unmatched_detections']} | "
            f"{v['C1_inconclusive_candidates']} | {'passa' if v['C1_block_pass'] else 'não passa'} | {v['C2_eligible_lines']} | "
            f"{_fmt(v['C2_fraction_within_tolerance'])} | {_fmt(v['C2_Q'])} | {_fmt(v['C2_p_Q_holm'])} | {'passa' if v['C2_block_pass'] else 'não passa'} |"
        )
    lines += [
        "",
        f"- **C1 (recuperação de frequências):** {family['C1_blocks_passing']}/{family['n_blocks']} blocos passam; "
        f"blocos com detecção sem correspondência no catálogo: {family['C1_blocks_with_unmatched_detection']} "
        f"(máximo pré-registrado {c['max_blocks_with_unmatched']}); replicado em todos os blocos: **{family['C1_replicated_all_blocks']}**.",
        f"- **C2 (concordância de coeficientes, tolerância {c['ratio_tolerance']:.0e}, fração mínima {c['coefficient_fraction_min']}, Q ≥ {c['q_min']}):** "
        f"{family['C2_blocks_passing']}/{family['n_blocks']} blocos passam; replicado em todos os blocos: **{family['C2_replicated_all_blocks']}**.",
        "- **C3 (compatibilidade estatística):** avaliada na execução M2 correspondente.",
    ]
    return "\n".join(lines)


def write_m3_run_report(run_dir: Path, manifest: Dict[str, Any], results: Dict[str, Dict[str, Any]], family: Dict[str, Any] | None = None) -> None:
...
| Candidatos cuja classificação mudaria usando z refinado | {res['freeze']['n_candidates_whose_decision_would_change_with_refined_z']} |
| Faixa inconclusiva do limiar (IC {ns['shuffle']['threshold_interval'].get('confidence_requested', '—') if ns['shuffle'].get('threshold_interval') else '—'}) | {('[' + _fmt(ns['shuffle']['threshold_interval']['z_low'], 4) + ', ' + _fmt(ns['shuffle']['threshold_interval']['z_high'], 4) + ']') if ns['shuffle'].get('threshold_interval') else '—'} |
| Candidatos inconclusivos quanto à detecção significativa (nulo primário) | {res['freeze'].get('n_inconclusive_primary', '—')} (dos quais com correspondência no catálogo: {ar.get('inconclusive_matching_catalog', '—')}) |
| Linhas claramente detectáveis; recuperação entre elas | {ar.get('clearly_detectable', '—')}; {_fmt(ar.get('recovery_among_clearly_detectable'))} |
| Concordância de coeficientes (critério C2): elegíveis; fração com abs(razão−1) ≤ tolerância | {ar['coefficient_agreement']['n_eligible'] if 'coefficient_agreement' in ar else '—'}; {_fmt(ar['coefficient_agreement']['fraction_within_tolerance']) if 'coefficient_agreement' in ar else '—'} |
...
| Detecções sem correspondência no catálogo; detecções por realização nula | {ar['unmatched_detections']}; {_fmt(ar['null_detections_mean'])} |
